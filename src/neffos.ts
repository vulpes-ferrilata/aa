...
class Message {
    ...
    isWait(): boolean {
        ...
        if (this.wait && this.wait[0] === waitIsConfirmationPrefix) {
            return true;
        }

        return !!this.wait && this.wait[0] === waitComesFromClientPrefix;
    }
...
class NSConn {
    ...
    async leaveAll(): Promise<Error | undefined> {
    ...
    disconnect(): Promise<Error | undefined> {
    ...
    async askRoomLeave(msg: Message): Promise<Error | undefined> {
...
type MessageHandlerFunc = (c: NSConn, msg: Message) => Error | undefined;


type Events = Map<string, MessageHandlerFunc>;
type Namespaces = Map<string, Events>;

function fireEvent(ns: NSConn, msg: Message): Error | undefined {
    if (msg.Event && ns.events.has(msg.Event)) {
        const event = ns.events.get(msg.Event);
        return event && event(ns, msg);
    }

    if (ns.events.has(OnAnyEvent)) {
        const event = ns.events.get(OnAnyEvent);
        if (event) {
            return event(ns, msg);
        }
        return;
...
function resolveNamespaces(obj: any, reject: (reason?: any) => void): Namespaces | undefined {
    if (isNull(obj)) {
        if (!isNull(reject)) {
...
function getEvents(namespaces: Namespaces, namespace: string): Events | undefined {
...
*/
function dial(endpoint: string, connHandler: any, options?: Options): Promise<Conn> {
...
function whenResourceOnline(endpoint: string, checkEvery: number, notifyOnline: (tries: number) => void) {
    ...
    const fetchOptions: RequestInit = { method: 'HEAD', mode: 'no-cors' };
...
class Conn {
    ...
    constructor(conn: WebSocket, namespaces: Namespaces) {
        ...
        const emptyNamespace = namespaces.get("")
        this.allowNativeMessages = hasEmptyNS && !!emptyNamespace && emptyNamespace.has(OnNativeMessage);
    ...
    handle(evt: MessageEvent): Error | undefined {
    ...
    private handleAck(data: WSData): Error | undefined {
        let typ = data[0];
        switch (typ) {
            case ackIDBinary:
    ...
    private handleMessage(data: WSData): Error | undefined {
    ...
    waitServerConnect(namespace: string): Promise<NSConn | undefined> {
        if (isNull(this.waitServerConnectNotifiers)) {
            this.waitServerConnectNotifiers = new Map<string, () => void>();
        }

        return new Promise(async (resolve, reject) => {
            this.waitServerConnectNotifiers && this.waitServerConnectNotifiers.set(namespace, () => {
                this.waitServerConnectNotifiers && this.waitServerConnectNotifiers.delete(namespace);
    ...
    private replyConnect(msg: Message): void {
        ...
        if (this.waitServerConnectNotifiers && this.waitServerConnectNotifiers.size > 0) {
            if (this.waitServerConnectNotifiers.has(msg.Namespace || "")) {
                const waitServerConnectNotifier = this.waitServerConnectNotifiers.get(msg.Namespace || "");
                waitServerConnectNotifier && waitServerConnectNotifier();
            }
        }
    }
    ...
    async askDisconnect(msg: Message): Promise<Error | undefined> {
        let ns = this.namespace(msg.Namespace || "");
        if (ns === undefined) { // it's already connected.
            return ErrBadNamespace;
        }
